...
import '../http/HttpService.dart'; // Importation du widget de boîte de recherche
...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  int selectedFoodCard = -1; // Indice de la catégorie d'aliments sélectionnée. -1 pour dire, qu'au départ, aucune catégorie est selectionnée.
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
              setState(() async {
                List<Food> foods = await Future.wait(foodList as Iterable<Future<Food>>);
                isAscendingOrder = !isAscendingOrder;
                if (isAscendingOrder) {
                  foods.sort((a, b) => a.price.compareTo(b.price)); // Tri en ordre croissant
                } else {
                  foods.sort((a, b) => b.price.compareTo(a.price)); // Tri en ordre décroissant
                }
              });
            },
          ),
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                /* InkWell permet d'afficher une animation d'encre
                              ...
                              */
                return InkWell(
                  child: FoodCard(dish.imagePath, dish.title, dish.description, dish.price, dish)
                  );
              },
            );
          } else {
  ...
  Widget FoodCard(
    ...
    return SingleChildScrollView(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  final snackBar = SnackBar(
                    content: Text('${food.title} added to Cart'), // Message de la barre d'informations
      ...
      child: Ink.image(
        ...
        fit: BoxFit.cover, // Utilisation de Hero pour des transitions d'images fluides entre les écrans
        child: Image.network(
          imagePath, // Chemin vers l'image à afficher
          width: MediaQuery.of(context).size.width / 5, // Largeur de l'image basée sur la largeur de l'écran
        ),
      ),
    ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            RawMaterialButton(
                onPressed: () async {
                  //foodList.removeWhere((food) => food.name.toUpperCase() != name.toUpperCase());
                  /*List<Food> foods = await Future.wait(foodList as Iterable<Future<Food>>);
                  for(int i=0; i<foods.length; i++){
                    if(!foods.contains("MEAT"))
                      //if(!foods.contains(name.toUpperCase()))
                      foods.removeAt(i);
                  }
                  foodList = foods as Future<List<Food>>;
                  print(foodList);*/
                },
